export class Ui {
  constructor() {
    ...
    this.gameDetails = document.querySelector(".gameDetails"); // Add this line
  }
      ...
      for (let i = 0; i < games.length; i++) {
        newGame += `
          <div class="game col-12 col-md-6 col-lg-4 bg-transparent p-0  text-center  mb-4">
            <div class="iner mx-3 text-white rounded-3 p-2" data-game-id="${games[i].id}">
              <figure class="m-3 ">
    ...
    let detailsHTML = `
      ...
      <div class="row datels mt-5">
        ...
        <figcaption class="col-12 col-lg-8 text-white">
          <h2 class="GameTitle fs-1 mb-5">Title: ${game.title}</h2>
          <h3>Category: <span>${game.genre}</span></h3> 
          <h3>Platform: <span>${game.platform}</span></h3>
          <h3 class="mb-5">Status: <span>${game.status}</span></h3>
          <p class="mb-5">${game.description}</p>
          <a href="${game.game_url}" class="showGame btn btn-outline-warning fs-2 mb-3" target="_blank">Show Game</a>
        </figcaption>
      </div>
    `;
